...
use cedar_elements::{
    cedar::{ErrorBoundedValue, PolarAlignAdvice},
...
impl PolarAnalyzer {
    ...
    // * The date/time and observer geographic location is known (valid
    //   hour_angle, latitude).
    // When certain other conditions are met, this function updates the
    // `polar_align_advice` state.
    pub fn process_solution(
        &mut self,
        boresight_pos: &CelestialCoord,
        hour_angle: f64,
        latitude: f64,
        motion_estimate: &Option<MotionEstimate>,
    ) {
        ...
        if motion_estimate.ra_rate.abs() > SIDEREAL_RATE * 0.3 {
            debug!(
                "Not updating polar alignment advice: \
                excessive ra_rate {}arcsec/sec",
        ...
        // evaluation; hours (doubled) above east or west horizon for polar
        // alignment elevation evaluation.
        const HA_TOLERANCE: f64 = 1.0;

        let dec = boresight_pos.dec;
        if dec > DEC_TOLERANCE || dec < -DEC_TOLERANCE {
            debug!(
                "Not updating polar alignment advice: declination {}deg",
                dec
            );
            return;
        }
        ...
        // Compute the angle formed by the declination drift rate at a right
        // angle to the adjusted_sidereal_rate. Degrees.
        let mut dec_drift_angle =
            (dec_rate / adjusted_sidereal_rate).atan().to_degrees();
        let mut dec_drift_angle_error = (dec_rate_error
            / adjusted_sidereal_rate)
            .atan()
            .to_degrees();
        ...
        if ha_hours > -HA_TOLERANCE && ha_hours < HA_TOLERANCE {
            // Near meridian. We can estimate polar alignment azimuth deviation
            // by declination drift method.
            ...
            // We express polar axis azimuth correction as positive angle
            // (clockwise looking down at mount from above) or
            // negative angle (counter-clockwise), rather than in
            // terms of east or west. This value is thus independent
            // of northern/southern hemisphere.
        ...
        if ha_hours > -6.0 && ha_hours < -6.0 + 2.0 * HA_TOLERANCE {
            ...
            // Northern hemisphere:
            // Boresight drifting south (star drifting north in FOV): polar axis
            // too high. Boresight drifting north (star drifting
            // south in FOV): polar axis too low.
            ...
            // Northern hemisphere:
            // Boresight drifting south (star drifting north in FOV): polar axis
            // too low. Boresight drifting north (star drifting
            // sourth in FOV): polar axis too high.
            altitude_correction = -dec_drift_angle;
        } else {
            debug!(
                "Not updating polar alignment advice: hour angle {}h",
                ha_hours
            );
            return;
        }
        ...
        self.polar_align_advice.altitude_correction = Some(ErrorBoundedValue {
            value: altitude_correction,
